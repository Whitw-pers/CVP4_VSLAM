...
while ~isMapInitialized && currFrame < length(imds.Files)
    ...
    % ORB-SLAM computes both homography and fundamental matrix,
...
while currFrame < length(imds.Files)
    ...
    %}
    % it runs using this example function from the mathworks example, so
    % error is inside stockKeyframeSelector somewhere
    numSkipFrames     = 20;
    numPointsKeyFrame = 90;
    scaleFactor = 1.2;
    % [localKeyframeIDs, currPose, worldPointsIdx, featureIdx, isKeyframe] = ...
    %     helperTrackLocalMap(worldPointSet, keyframeSet, worldPointsIdx, ...
    %     featureIdx, currPose, currFeatures, currPoints, intrinsics, scaleFactor, ...
    %     isLastKeyframe, lastKeyframe, currFrame, numSkipFrames, numPointsKeyFrame);
    % 
    [localKeyframeIDs, currPose, worldPointsIdx, featureIdx, isKeyframe] = ...
        helperTrackLocalMapOKVIS(worldPointSet, keyframeSet, worldPointsIdx, ...
        featureIdx, currPose, currFeatures, currPoints, intrinsics, scaleFactor, ...
        isLastKeyframe, lastKeyframe, currFrame, numSkipFrames, numPointsKeyFrame);
