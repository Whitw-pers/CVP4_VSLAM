function [localKeyFrameIds, currPose, mapPointIdx, featureIdx, isKeyFrame] = ...
    helperTrackLocalMapOKVIS(mapPoints, vSetKeyFrames, mapPointIdx, ...
...
isKeyFrame = checkKeyFrame(currPoints,matchedImagePoints);
...
function isKeyFrame = checkKeyFrame(currPoints, matchedFeatures2D)
...
allFeatures2D = currPoints.Location;


% Maximum x-coordinate
maxX = max(allFeatures2D(:,1));

% Maximum y-coordinate
maxY = max(allFeatures2D(:,2));

% Minimum x-coordinate
minX = min(allFeatures2D(:,1));

% Minimum y-coordinate
minY = min(allFeatures2D(:,2));

% Maximum x-coordinate for matched points
maxMatchX = max(matchedFeatures2D(:,1));

% Maximum y-coordinate for matched points
maxMatchY = max(matchedFeatures2D(:,2));

% Minimum x-coordinate for matched points
minMatchX = min(matchedFeatures2D(:,1));

% Minimum y-coordinate for matched points
minMatchY = min(matchedFeatures2D(:,2));

Ratio = ( (maxMatchX - minMatchX) * (maxMatchY - minMatchY) ) / ( (maxX - minX) * (maxY - minY) );
